/**
 * 从 design-tokens.json 生成 primitives.css + semantic.css
 * 用法: node scripts/build-primitives-semantic-css.mjs
 */
import fs from 'fs';
...
/** Figma 路径 "Color/brand/500" → --color-brand-500 */
...
const primCss = `/**
 * Primitives — 底层色板（自 design-tokens.json → primitives）
 */

:root {
${primList.map(({ key, cssVal }) => `  --${key}: ${cssVal};`).join('\n')}
}
`;

const semCss = `@import "./primitives.css";

/**
 * Semantic — 语义 Token；颜色引用 primitives 变量
 */

:root {
${semList.map(({ key, cssVal }) => `  --${key}: ${cssVal};`).join('\n')}
}
`;

fs.writeFileSync(path.join(root, 'primitives.css'), primCss, 'utf8');
fs.writeFileSync(path.join(root, 'semantic.css'), semCss, 'utf8');

console.log(`Wrote primitives.css (${primList.length} vars), semantic.css (${semList.length} vars)`);
